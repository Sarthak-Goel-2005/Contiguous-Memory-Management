...
void best_fit(string id,int size){
    if(size>free_size){
    ...
    int best_size=0;
    while(temp!=NULL){
        if(temp->id=="Free" && temp->size>=size){
            if(best_size) best_size=min((temp->size),best_size);
            else best_size=temp->size;
        }
        temp=temp->next;
    }
    if(best_size==0){
    ...
    temp=new Node(id,size);
    free_size-=size;
    if(head->id=="Free" && (head->size)==(best_size)){
        ...
        Node* curr=head;
        Node* prev=NULL;
        while(curr->id!="Free" || (curr->size)!=(best_size)){
            prev=curr;
            curr=curr->next;
        }
                                     //Inserting node at between two nodes of Linked List!
        prev->next=temp;
        temp->next=curr;
        temp->loc=curr->loc;
